...
async def settings_cmd(message: Message):
    await message.answer(
        phrases.get_value("settings_menu"), reply_markup=get_settings_keyboard()
    )
...
async def process_set_timer(callback: CallbackQuery):
    await callback.message.edit_text(
        phrases.get_value("choose_timer"), reply_markup=get_timer_keyboard()
    )
...
async def save_timer(callback: CallbackQuery, bot: Bot):
    ...
    await callback.message.edit_text(
        phrases.get_value("timer_saved").format(hours=hours)
    )
...
async def process_set_radius(callback: CallbackQuery):
    await callback.message.edit_text(
        phrases.get_value("choose_radius"), reply_markup=get_radius_keyboard()
    )
...
async def save_radius(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        phrases.get_value("radius_saved").format(radius=radius)
    )
